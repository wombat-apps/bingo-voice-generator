...
APP_NAME=${APP_NAME:-BingoVoiceGenerator}
BUNDLE_ID=${BUNDLE_ID:-com.example.bingo-voice-generator}
...
# Load Sparkle config if available
...
# Build with ENABLE_SPARKLE flag if configured
SWIFT_FLAGS=""
if [[ "$ENABLE_SPARKLE" == "1" && -n "$SPARKLE_FEED_URL" && -n "$SPARKLE_PUBLIC_ED_KEY" ]]; then
  SWIFT_FLAGS="-Xswiftc -DENABLE_SPARKLE"
  echo "Building with Sparkle enabled"
fi
...
# Build Sparkle Info.plist entries if configured
...
<dict>
    ...
    <key>GitCommit</key><string>${GIT_COMMIT}</string>${SPARKLE_PLIST_ENTRIES}
</dict>
</plist>
PLIST
...
# Sign bundled executables in Resources and resource bundles (e.g., ffmpeg)
sign_bundled_executables() {
  # Find all executable files in Resources (including inside .bundle directories)
  while IFS= read -r -d '' exe; do
    echo "Signing bundled executable: $exe"
